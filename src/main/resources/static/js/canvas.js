...
const paintToggle = () => {
  ...
  for (let row = 0; row < TOGGLE_ROWS; row++) {
    for (let col = 0; col < TOGGLE_COLS; col++) {
      ...
      ctx.fillText(text, col * TILE_WIDTH + 10, (row + 0.8) * TILE_HEIGHT);
    }
  }
};
...
const paintOnClick = event => {
  ...
  let tsetting = flags[row];
  if (col == 1) {
    // case - the user selected 'on'
    ctx.fillStyle = "#A7C7E7";
    ctx.fillRect(col * TILE_WIDTH, row * TILE_HEIGHT, TILE_WIDTH, TILE_HEIGHT);
    ctx.fillStyle = "26222A"
    ctx.fillText("on", col * TILE_WIDTH + 10, (row + 0.8) * TILE_HEIGHT);
    ...
    ctx.fillStyle = "26222A"
    ctx.fillText("off", 2 * TILE_WIDTH + 10, (row + 0.8) * TILE_HEIGHT);
    ...
    if (!selection.includes(tsetting)) {
      selection.push(tsetting);
    }
    ...
    ctx.fillRect(col * TILE_WIDTH, row * TILE_HEIGHT, TILE_WIDTH, TILE_HEIGHT);
    ctx.fillStyle = "26222A"
    ctx.fillText("off", col * TILE_WIDTH + 10, (row + 0.8) * TILE_HEIGHT);
    // TODO: color the current tile using ctx.fillRect() with whichever select
    // you chose in paintToggle(). Remember that after coloring,
    // you're going to want to draw the text again using ctx.fillText()
    ctx.fillStyle = "FFFDD0";
    ctx.fillRect( 1 * TILE_WIDTH, row * TILE_HEIGHT, TILE_WIDTH, TILE_HEIGHT);
    ctx.fillStyle = "26222A"
    ctx.fillText("on", 1 * TILE_WIDTH + 10, (row + 0.8) * TILE_HEIGHT);
    ...
    const index = selection.indexOf(tsetting);
    if (index > -1) {
